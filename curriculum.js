...
const rect=(w,h,labels=true)=>svg(`<rect x="65" y="30" width="230" height="110" rx="2" fill="#eee6f9" stroke="#7850aa" stroke-width="2"/>${labels?txt(180,22,w+' cm')+txt(327,90,h+' cm'):''}${txt(180,170,'Diagram not to scale')}`);
...
function placeQuestion(c,year,t){
 const n=c.int(t===1?100:1000,year===3?10000:100000),p=c.int(0,String(n).length-1),place=10**p;
...
for(const year of [3,4]){
 ...
 skill(p+'-patterns',year,'Numbers','Compare & find patterns',(c,t)=>{
  const max=year===3?10000:100000,step=c.pick([10,100,1000]),a=c.int(1,max-4*step);
...
skill('p3-addsub',3,'Operations','Add & subtract',(c,t)=>{
 const a=c.int(t===1?20:100,t===1?99:9999),b=c.int(10,t===1?99:9999),add=c.random()<.5;
...
skill('p3-tables',3,'Operations','Times tables 6–9',(c,t)=>{const a=c.int(6,9),b=c.int(2,10);return c.random()<.5?num(`${a} × ${b} = ?`,a*b,`Use a known fact such as 5 × ${b}, then add the extra groups.`):num(`${a*b} ÷ ${a} = ?`,b,`Which number multiplied by ${a} makes ${a*b}?`);});
function operation(c,year,t,divide=false){
 const b=c.int(2,9),max=year===3?999:9999;
 if(divide){const a=c.int(12,Math.floor(max/b));return num(`${a*b} ÷ ${b} = ?`,a,'Share the largest place first; regroup any remainder into the next place.');}
...
for(const year of [3,4]) skill(`p${year}-remainder`,year,'Operations','Division with remainders',(c,t)=>{
 const b=c.int(2,9),a=c.int(12,year===3?999:9999),r=a%b,quo=Math.floor(a/b),ans=`${quo} R ${r}`;
...
skill('p3-equivalent',3,'Fractions','Equivalent & simplest fractions',(c,t)=>{
 const d=c.pick([2,3,4,6]),n=c.int(1,d-1),k=c.int(2,Math.floor(12/d));
 if(c.random()<.5)return num(`${n}/${d} = ?/${d*k}. Find the missing numerator.`,n*k,'Multiply numerator and denominator by the same number.');
...
skill('p3-fraccompare',3,'Fractions','Compare unlike fractions',(c,t)=>{
 const d=c.int(2,12),e=c.int(2,12),n=c.int(1,d-1),m=c.int(1,e-1),a=n*e,b=m*d;
 return choice(`Compare ${n}/${d} and ${m}/${e}.`,a>b?'>':a<b?'<':'=',['<','=','>'],'Use equal-sized wholes. Rewrite with a shared denominator to compare.');
},'p3-equivalent');
function fracCalc(c,year){
 let d,e,n,m,add;
 // P3 related denominators and total within one whole; P4 at most two denominators, each ≤12.
 for(let i=0;i<100;i++){
 d=c.pick(year===3?[2,3,4,6]:[2,3,4,5,6,8,10,12]);e=year===3?d*c.int(1,Math.floor(12/d)):c.int(2,12);n=c.int(1,d-1);m=c.int(1,e-1);add=c.random()<.5;
...
skill('p3-fracops',3,'Fractions','Add & subtract related fractions',c=>fracCalc(c,3),'p3-equivalent');
skill('p3-money',3,'Everyday maths','Money & change',(c,t)=>{
 const a=c.int(150,2000),b=c.int(50,Math.min(a,1000)),add=c.random()<.5;
...
skill('p3-measure',3,'Everyday maths','Length, mass & capacity',(c,t)=>{
 ...
 if(c.random()<.5)return num(`Convert ${w} ${a} ${r} ${b} to ${b}.`,w*k+r,`1 ${a} = ${k} ${b}. Convert the larger units, then add.`, '',b);
...
skill('p3-time',3,'Everyday maths','Time, seconds & 24-hour clock',(c,t)=>{
 const type=c.int(0,4),start=c.int(420,1000),dur=c.int(15,120);
...
skill('p3-area',3,'Geometry','Area & perimeter',(c,t)=>{
 const w=c.int(3,12),h=c.int(2,10),area=c.random()<.5;
 if(t>=2&&c.random()<.35){const W=c.int(6,12),H=c.int(5,10),a=c.int(2,W-2),b=c.int(2,H-2);
...
skill('p3-angles',3,'Geometry','Compare with a right angle',(c,t)=>{
 const deg=c.pick([30,45,60,90,120,135,150]),ans=deg===90?'A right angle':deg<90?'Smaller than a right angle':'Greater than a right angle';
 return choice('Compare this angle with a square corner.',ans,['Smaller than a right angle','A right angle','Greater than a right angle'],'Imagine fitting the corner of a sheet of paper into the angle.',angleDrawing(deg));
});
skill('p3-lines',3,'Geometry','Parallel & perpendicular lines',(c,t)=>{
 ...
 return choice('How are the two lines related?',ans,['Parallel','Perpendicular','Neither'],'Parallel lines stay equally far apart. Perpendicular lines meet at a right angle.',svg(s));
});
function graph(c,kind){
 const names=kind==='bar'?['Hana','Mei','Arif','Ben']:['Mon','Tue','Wed','Thu'],step=c.pick([2,5,10]),v=names.map(()=>c.int(1,8)*step),top=8*step,x0=55,y0=170;
 ...
 return c.random()<.5?num(`How many books ${kind==='bar'?'did '+names[i]+' borrow':'were borrowed on '+names[i]}?`,v[i],`Each interval on the vertical scale represents ${step} books.`,svg(s,360,215),'books'):num(`What is the difference between ${names[i]} and ${names[j]}?`,Math.abs(v[i]-v[j]),'Read both values from the scale, then subtract the smaller from the larger.',svg(s,360,215),'books');
}
skill('p3-bars',3,'Data','Read scaled bar graphs',c=>graph(c,'bar'));
skill('p3-story',3,'Problem solving','Two-step word problems',(c,t)=>{
 const boxes=c.int(3,9),each=c.int(6,9),away=c.int(2,each*2);
...
skill('p4-round',4,'Numbers','Round whole numbers',(c,t)=>{const n=c.int(1000,99999),unit=c.pick([10,100,1000]);return num(`Round ${n} to the nearest ${unit}.`,Math.round(n/unit)*unit,'Look at the next digit: 5 or more rounds up. Use ≈ for an approximate value.');},'p3-place');
skill('p4-factors',4,'Numbers','Factors & common multiples',(c,t)=>{
 const a=c.int(2,9),b=c.int(2,9);
 if(c.random()<.5){const x=a*c.int(2,10),y=a*c.int(2,10),correct=a,wrong=Array.from({length:9},(_,i)=>i+1).filter(v=>x%v||y%v);return choice(`Which is a common factor of ${x} and ${y}?`,correct,[correct,...wrong.slice(0,3)],'A common factor divides each number exactly.');}
...
skill('p4-multiply',4,'Operations','Multiply larger numbers',(c,t)=>{
 if(c.random()<.5)return operation(c,4,t,false);
 const a=c.int(t===1?12:100,t===1?99:999),b=c.int(10,99);return num(`${a} × ${b} = ?`,a*b,`Multiply ${a} by the tens and by the ones of ${b}; add the partial products.`);
},'p3-muldiv');
...
skill('p4-mixed',4,'Fractions','Mixed & improper fractions',(c,t)=>{
 const d=c.int(2,12),n=c.int(1,d-1),w=c.int(1,5),ans=`${w} ${f(n,d)}`;
 if(c.random()<.5)return num(`Write ${w} ${n}/${d} as ?/${d}. What is the numerator?`,w*d+n,'Multiply the whole number by the denominator, then add the numerator.');
...
skill('p4-fracset',4,'Fractions','Fractions of a set',(c,t)=>{
 const d=c.int(2,12),n=c.int(1,d-1),k=c.int(2,12);return num(`There are ${d*k} beads. ${n}/${d} of them are blue. How many beads are blue?`,n*k,`Share the whole set into ${d} equal groups, then take ${n} groups.`,'','beads');
},'p3-muldiv');
skill('p4-fracops',4,'Fractions','Add & subtract unlike fractions',c=>fracCalc(c,4),'p3-fracops');
skill('p4-decimalplace',4,'Decimals','Decimal places & rounding',(c,t)=>{
 const n=c.int(1001,9999),s=(n/1000).toFixed(3),kind=c.int(0,2),place=c.pick([1,2,3]);
...
skill('p4-decimalfraction',4,'Decimals','Fractions ↔ decimals',(c,t)=>{
 const d=c.pick([2,4,5,10,20,25,50,100]),n=c.int(1,d-1);
 if(c.random()<.5){const q=num(`Write ${n}/${d} as a decimal.`,n/d,'Make an equivalent fraction with denominator 10 or 100.');q.dec=true;return q;}
...
skill('p4-decimalops',4,'Decimals','Calculate with decimals',(c,t)=>{
 const a=c.int(100,999),b=c.int(10,99),op=c.int(0,3),k=c.int(2,9);let q;
...
skill('p4-decimalquotient',4,'Decimals','Division & rounded answers',(c,t)=>{
 const a=c.int(1,80),b=c.int(2,9),dp=c.pick([1,2]),q=num(`${a} ÷ ${b} = ? Round your answer to ${dp} decimal ${dp===1?'place':'places'}.`,Math.round(a/b*10**dp)/10**dp,'Continue dividing into decimal places. Use the next digit to round.');q.dec=true;return q;
},'p4-divide');
skill('p4-dimension',4,'Geometry','Find missing lengths',(c,t)=>{
 const w=c.int(3,12),h=c.int(2,10),area=c.random()<.5;
 if(c.random()<.4)return num(`A square has ${area?'area '+w*w+' cm²':'perimeter '+4*w+' cm'}. How long is one side?`,w,area?'Find a whole number that multiplied by itself gives the area.':'A square has four equal sides. Divide the perimeter by four.','','cm');
...
skill('p4-composite',4,'Geometry','Composite area & perimeter',(c,t)=>{
 const W=c.int(7,14),H=c.int(6,10),a=c.int(2,W-3),b=c.int(2,H-3),area=c.random()<.5;
 ...
 return num(`Find the ${area?'area':'perimeter'} of this L-shaped garden.`,area?W*H-a*b:2*(W+H),area?'Subtract the missing corner from the large rectangle.':'Find the missing lengths; add every outside edge.',s,area?'cm²':'cm');
},'p3-area');
skill('p4-angles',4,'Geometry','Measure & name angles',(c,t)=>{
 const deg=10*c.int(2,16);return num('Use the scale to measure ∠ABC. Start at 0 on ray BC.',deg,'The middle letter B is the vertex. Read where ray BA meets the scale.',angleDrawing(deg,true),'°');
},'p3-angles');
...
skill('p4-tables',4,'Data','Complete & interpret tables',(c,t)=>{
 ...
 const vis=`<table class="data-table"><caption>Books borrowed</caption><thead><tr><th>Mon</th><th>Tue</th><th>Wed</th><th>Total</th></tr></thead><tbody><tr><td>${a}</td><td>?</td><td>${d}</td><td>${total}</td></tr></tbody></table>`;
 return num('How many books were borrowed on Tuesday?',b,'Subtract the two known days from the total.',vis,'books');
},'p3-bars');
skill('p4-linegraphs',4,'Data','Read line graphs',c=>graph(c,'line'),'p3-bars');
skill('p4-piecharts',4,'Data','Read pie charts',(c,t)=>{
 const k=c.int(0,2),names=['Reading','Drawing','Swimming'],n=c.int(3,12),values=[2*n,n,n];
 ...
 return num(`${4*n} children chose one favourite activity each. How many chose ${names[k].toLowerCase()}?`,values[k],'Reading takes half the circle. Drawing and swimming each take one quarter.',s,'children');
},'p4-fracset');
